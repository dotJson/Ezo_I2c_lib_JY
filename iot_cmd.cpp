...
void process_command(const String &string_buffer, Ezo_board device_list[], uint8_t device_list_len, Ezo_board* &default_board){
  ...
  if (string_buffer == "LIST") {                    //if our command is list
    Serial.println("FUCKERz > ");
    Serial.println(string_buffer);
    TelnetStream.println(string_buffer);
    ...
    Serial.println("WE ARE HERE: 2");                                              //if it contains a colon character
      ...
      Serial.println("WE ARE HERE: 3");                                             //if its valid
        ...
        for (uint8_t i = 0; i < device_list_len; i++) {
          ...
          Serial.println("WE ARE HERE: 4");               //if the address matches one of the boards in the list
        ...
        if (addr_found) { 
        Serial.println("WE ARE HERE: 5");                                          //then send the rest of the command to that board
          default_board->send_cmd(string_buffer.substring(index + 1).c_str());
        } else {  
        Serial.println("WE ARE HERE: 6");                                                  //otherwise print that we didnt find
          Serial.print("No device named ");
          Serial.println(name_to_find);
          TelnetStream.print("No device named ");
          TelnetStream.println(name_to_find);
          return;
        }
      } else {
        Serial.println("WE ARE HERE: 7");
        Serial.println("Invalid name");
        TelnetStream.print("Invalid name");
      }
    }
    else {  
    Serial.println("WE ARE HERE: 8");                                                        //if theres no colon just pass the command to the default board
      default_board->send_cmd(string_buffer.c_str());
    }

    select_delay(string_buffer);                                   //wait for some time depending on the command
Serial.println("WE ARE HERE: 9");
    receive_and_print_response(*default_board);
  }
  Serial.println("WE ARE HERE: 10");
}
...
void list_devices(Ezo_board device_list[], uint8_t device_list_len, Ezo_board* default_board) {
  for (uint8_t i = 0; i < device_list_len; i++) {        //go thorugh the list of boards
    if (default_board == &device_list[i]) {              //if its our default board
      ...
      TelnetStream.print("--> ");                             //print the pointer arrow
    } else {                                            //otherwise
      Serial.print(" -- ");                              //print a normal dash
      TelnetStream.print(" -- ");                              //print a normal dash
    }
